...
test("create a CkbSimpleAccount instance", async (t) => {
  ...
  const generator = await readFile(dummySmtGeneratorPath);
  const validator = await readFile(dummySmtValidatorPath);
  ...
  console.log(new Int8Array(validator));
  console.log(normalizers.NormalizeOutPoint(validator_outpoint));
  const capacity = 100;
  const config = {
    validator: validator,
    generator: generator,
